  ...
  return (
    <div className="h-full flex flex-col md:flex-row gap-6 max-w-7xl mx-auto w-full animate-in fade-in duration-500">
      ...
      <div className="flex-none md:w-72 flex flex-col gap-2">
         {/* Page Header */}
         <div className="mb-4 md:mb-8 bg-white dark:bg-slate-800/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden">
             <div className="absolute top-0 right-0 -mt-10 -mr-10 h-32 w-32 bg-indigo-500/10 rounded-full blur-2xl pointer-events-none"></div>
             <div className="relative z-10 flex items-center gap-4">
                 <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white shadow-lg shadow-indigo-200 dark:shadow-none shrink-0">
                 ...
                 <div>
                     ...
                     <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-0.5">Configuration</p>
                 </div>
             </div>
         </div>
         ...
         <div className="flex md:flex-col gap-2 overflow-x-auto md:overflow-visible no-scrollbar pb-2 md:pb-0">
            {tabs.map(tab => (
                <button 
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 text-left shrink-0 md:shrink border ${
                      activeTab === tab.id 
                      ? 'bg-white dark:bg-slate-800 shadow-md border-indigo-100 dark:border-indigo-900/30 transform md:scale-105 z-10' 
                ...
                >
                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 transition-colors duration-300 ${
                        activeTab === tab.id
                        ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400'
                        : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'
                    }`}>
                        <tab.icon className={`h-5 w-5 ${activeTab === tab.id ? 'stroke-[2px]' : 'stroke-[1.5px]'}`} />
                    </div>
                    <div className="hidden md:block">
                        <h4 className={`font-bold transition-colors ${activeTab === tab.id ? 'text-indigo-900 dark:text-indigo-100' : 'text-slate-600 dark:text-slate-400'}`}>{tab.label}</h4>
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-800/50 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden relative">
        
        {/* Global Notification Overlay */}
        {notification && (
            <div className="absolute top-0 left-0 right-0 z-50 p-4 animate-in slide-in-from-top-4 fade-in duration-300">
                <div className={`max-w-md mx-auto p-4 rounded-2xl flex items-center gap-3 text-sm font-bold shadow-xl border backdrop-blur-md ${
                    notification.type === 'success' 
                    ? 'bg-emerald-50/90 dark:bg-emerald-900/90 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800' 
                    : 'bg-rose-50/90 dark:bg-rose-900/90 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-800'
                }`}>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-6 md:p-10 relative">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-6">
                            ...
                            <div className="relative group">
                                <input
                                    ...
                                    className="block px-5 pb-3 pt-6 w-full text-base font-bold text-slate-900 dark:text-white bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 appearance-none focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 peer transition-all shadow-sm"
                                    placeholder=" "
                                />
                                <label htmlFor="messName" className="absolute text-sm text-slate-500 dark:text-slate-400 duration-300 transform -translate-y-3 scale-75 top-4 z-10 origin-[0] left-5 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-3 peer-focus:text-indigo-600 dark:peer-focus:text-indigo-400 font-bold pointer-events-none">Mess Name</label>
                            </div>

                            {/* Modern Input: Meal Price */}
                            <div className="relative group">
                                <div className="absolute left-0 top-0 bottom-0 w-16 flex items-center justify-center font-black text-slate-400 dark:text-slate-500 text-lg border-r border-slate-100 dark:border-slate-700 pointer-events-none z-10">
                                    ৳
                                </div>
                                <input
                                    type="number"
                                    ...
                                    className="block pl-20 pr-5 pb-3 pt-6 w-full text-base font-bold text-slate-900 dark:text-white bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 appearance-none focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 peer transition-all shadow-sm"
                                    placeholder=" "
                                />
                                <label htmlFor="mealPrice" className="absolute text-sm text-slate-500 dark:text-slate-400 duration-300 transform -translate-y-3 scale-75 top-4 z-10 origin-[0] left-20 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-3 peer-focus:text-indigo-600 dark:peer-focus:text-indigo-400 font-bold pointer-events-none">Default Meal Price</label>
                            </div>
                            
                            {/* Modern Input: YouTube Video URL */}
                            <div className="relative group">
                                <input
                                    type="text"
                                    id="youtubeVideoUrl"
                                    ...
                                    placeholder=" "
                                />
                                <label htmlFor="youtubeVideoUrl" className="absolute text-sm text-slate-500 dark:text-slate-400 duration-300 transform -translate-y-3 scale-75 top-4 z-10 origin-[0] left-5 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-3 peer-focus:text-indigo-600 dark:peer-focus:text-indigo-400 font-bold pointer-events-none">Dashboard Background Video URL (Optional)</label>
                            </div>
                        </div>

                        <Button type="submit" isLoading={saving} className="w-full sm:w-auto px-8 py-4 rounded-2xl text-base font-bold shadow-xl shadow-indigo-200 dark:shadow-indigo-900/20 bg-indigo-600 hover:bg-indigo-700 transition-all hover:-translate-y-1">
            ...
            {/* --- SECURITY TAB --- */}
            {activeTab === 'security' && (
                <div className="animate-in fade-in slide-in-from-right-8 duration-500 max-w-2xl space-y-8">
                    <div className="mb-6">
                    ...
                    {/* Backup Card */}
                    <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl p-8 shadow-xl shadow-emerald-500/20 text-white relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-emerald-50 mb-8 max-w-sm">Export your entire system configuration as a JSON file to keep it safe.</p>
                            <button onClick={exportData} className="px-6 py-3 bg-white text-emerald-700 hover:bg-emerald-50 rounded-xl font-bold transition-colors active:scale-95 shadow-sm flex items-center gap-2">
                    ...
                    {/* Danger Zone */}
                    <div className="bg-rose-50 dark:bg-rose-900/10 border-2 border-dashed border-rose-200 dark:border-rose-900/40 rounded-3xl p-8 relative overflow-hidden">
                        ...
                        <div className="bg-white dark:bg-slate-900/50 p-6 rounded-2xl border border-rose-100 dark:border-rose-900/30">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                                ...
                                <button 
                                    ...
                                    className="px-6 py-3 bg-rose-600 disabled:bg-slate-300 disabled:text-slate-500 hover:bg-rose-700 text-white rounded-xl font-bold transition-all flex items-center justify-center gap-2"
                                >
                ...
                <div className="animate-in fade-in slide-in-from-right-8 duration-500 h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar pb-10">
                        {logs.length === 0 ? (
                            ...
                            <div className="relative before:absolute before:inset-0 before:ml-6 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-slate-200 before:via-slate-200 before:to-transparent dark:before:from-slate-700 dark:before:via-slate-700 py-4">
                                {logs.map((log, index) => (
                                    <div key={log.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active mb-8 transition-all hover:-translate-y-1">
                                        ...
                                        {/* Timeline Dot & Icon */}
                                        <div className={`flex items-center justify-center w-12 h-12 rounded-full border-4 border-white dark:border-slate-800 shadow-md shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 ${getLogColor(log.action)}`}>
                                            {getLogIcon(log.action)}
                                        </div>

                                        {/* Content Box */}
                                        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                                            <div className="flex items-center justify-between mb-2">
                                                <h4 className="font-bold text-slate-800 dark:text-slate-200">{log.action}</h4>
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1 bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded-md">
                                            ...
                                            <div className="flex items-center gap-2 pt-3 border-t border-slate-50 dark:border-slate-800 min-w-0">
                                                ...
                                                <span className="text-xs font-medium text-slate-500 truncate" title={log.performedBy}>{log.performedBy}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                ...
                <div className="animate-in fade-in slide-in-from-right-8 duration-500 h-full flex flex-col">
                    <div className="mb-8 shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-slate-500 dark:text-slate-400">Configure default daily meal selections for each member.</p>
                        </div>
                        <Button onClick={saveAutoMeals} isLoading={saving} className="px-6 py-3 rounded-2xl shadow-lg shadow-emerald-200 dark:shadow-none bg-emerald-600 hover:bg-emerald-700 text-white shrink-0">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar pb-10">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {members.map(member => {
                                ...
                                return (
                                    <div key={member.id} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-5 rounded-3xl flex items-center justify-between gap-4 shadow-sm hover:shadow-md transition-shadow">
                                        <div className="flex items-center gap-3 min-w-0 flex-1">
                                            <div className="h-12 w-12 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-slate-500 overflow-hidden shrink-0">
                                                {member.photoBase64 ? (
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <h4 className="font-bold text-slate-800 dark:text-slate-200 truncate" title={member.fullName}>{member.fullName}</h4>
                                        ...
                                        <div className="flex flex-col gap-2 shrink-0">
                                            ...
                                            <div className="flex items-center justify-end gap-3">
                                                ...
                                                <button 
                                                    ...
                                                    className={`w-12 h-6 rounded-full relative transition-colors duration-300 focus:outline-none ${defaultMeals.dinner ? 'bg-indigo-500' : 'bg-slate-200 dark:bg-slate-700'}`}
                                                >
